...
const StackCardInternal = React.memo(function StackCardInternal({
  ...
  const x = useTransform(scrollProgress, [0, 1], [naturalX, naturalX - maxScroll]);
  ...
  const yDrop = useTransform(stackDepth, [0, cardWidth], [0, isMobile ? 6 : 12]);
  const opacity = useTransform(stackDepth, [0, cardWidth * 1.2], [1, 0.08]);
  const blur = useTransform(stackDepth, [0, cardWidth * 1.2], [0, 10]);
...
const LayerStack = React.forwardRef<HTMLDivElement, LayerStackProps>(function LayerStack(
  {
    children,
    className,
    cardWidth = 320,
    ...
    <div ref={ref} className={cn("w-full select-none", className)}>
      ...
      >
        {cards.map((card, i) => {
          ...
          const renderedWidth =
            lastCardFullWidth && isLastCard && containerWidth > 0 ? containerWidth : cardWidth;
      ...
      <div className="mt-4 flex flex-col items-center gap-2">
        ...
        />
        <p className="text-[10px] font-medium tracking-[0.14em] uppercase text-foreground/60">
